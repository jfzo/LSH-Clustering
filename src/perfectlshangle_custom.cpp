	...
	*/
	...
	for(int i=0; i < data.instances()-1; i++){
		...
		for(int j=i+1; j < data.instances(); j++){
            ...
            double anglehat = 0.0;
#if SIMESTMOD == 1
            anglehat = pow(1 - pow(1-est_p1,1.0/nbands) ,1.0/r); // --> MOD 1
#endif
#if SIMESTMOD == 2
            anglehat = pow(est_p1 , 1.0/(matches*r) ); // --> MOD 2
#endif
			simhat[i][j] = cos( (1-anglehat) *M_PI/2 );
